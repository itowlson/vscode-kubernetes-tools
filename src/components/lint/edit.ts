...
export function merge(edit: vscode.WorkspaceEdit, document: vscode.TextDocument, parsedDocument: kp.ResourceParse[], into: kp.ResourceParse | kp.MapValue | kp.MapTraversalEntry, value: object): void {
    ...
    if (languageId === 'yaml') {
        if (isEmptyMap) {
            const parentMap = ancs[0].value as kp.MapValue;
...
function objectOf(ast: { [key: string]: kp.ResourceMapEntry }): object {
    const o: any = {};
...
function jsValueOfRME(rme: kp.ResourceMapEntry): any {
    return jsValueOf(rme.value);
}

function jsValueOf(v: kp.Value): any {
    switch (v.valueType) {
...
function mapNodeOf(node: kp.MapTraversalEntry | kp.MapValue | kp.ResourceParse): kp.MapValue | undefined {
    if (isMapValue(node)) {
        return node;
    }
